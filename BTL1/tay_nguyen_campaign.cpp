        ...
        while (j < MAX_LINE_LENGTH) {   
          ...
          if (i == 0) {
            if (sum <= 0) {
              ...
              LF1[len++] = min(sum, 1000);
            }            
          }else if (i == 1) {
            if (sum <= 0) {
              LF1[len++] = 0;
            }else {
              LF2[len++] = min(sum, 1000);
            }            
          }          
        }        
      } else if (i == 2 || i == 3 || i == 4) {
...
// Task 5
void backtrack(int cells[25], int selected_cells[5], int selected_size, int index, int sum, int shortfall) {
  if (selected_size == 5) {
  ...
  for (int i = index; i < 25; i++) {
    // Pruning: If the minimum possible sum already exceeds bestSum, stop exploring
    if (sum + cells[i] * (5 - selected_size) >= bestSum) {
      return; // Stop further recursion
    }

  // Store the value and index
  selected_cells[selected_size] = cells[i];      

  // Recursively select the next cell
  backtrack(cells, selected_cells, selected_size + 1, i + 1, sum + cells[i], shortfall);

  // No need to manually reset values; they get overwritten in the next iteration
  }
}
